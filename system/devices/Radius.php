    ...
    function remove_customer($customer, $plan)
    {
        if (!empty($plan['plan_expired'])) {
            ...
            $this->customerDeactivate($customer['username'], true);
             //remove everything logged in with vouchers
             $vcr = ORM::for_table('tbl_voucher')
             ->where('code', $customer['username'])
             ->where('status', '3')
             ->find_one();
             if ($vcr) { 
             $cust = ORM::for_table('tbl_customers')
             ->where('username', $customer['username'])
             ->where('created_by', '9999')
             ->find_one();
             $usrchrg = ORM::for_table('tbl_user_recharges')
             ->where('username', $customer['username'])
             ->find_one();
             $this->customerDelete($customer['username']);
             $vcr->delete();
             $cust->delete();
             $usrchrg->delete();
             }           
